...
serPort = serial.Serial("COM4",      #port
                    9600,              #baudrate
...
'''try:
    ...
    sock.connect((ur_robot_ip, ur_robot_port))
...
coordinate_format = 'movej,{},{},{},{},{},{}\n'
home_pos            = [0.225, 0.420, -0.268, 2.063, -2.370,0]
coordinate_c1       = [0.324, 0.420, -0.268, 2.063, -2.370, 0]
coordinate_c1_dwn   = [0.324, 0.420, -0.3, 2.063, -2.370, 0]
coordinate_c2       = [0.428, 0.420, -0.268, 2.068, -2.370, 0]
coordinate_c2_dwn   = [0.428, 0.420, -0.3, 2.068, -2.370, 0]
coordinate_c3       = [0.518, 0.420, -0.268, 2.062, -2.370, 0]
coordinate_a1       = [0.582, 0.420, -0.268, 20.063,-2.370, 0]
coordinate_a2       = [0.582, 0.470, -0.268, 20.063,-2.370, 0]
acc = 2 #acceleration
vel = 2 #velocity
    ...
    while cap.isOpened():
        ...
        for detection in results.xyxy:  # xyxy format: [x1, y1, x2, y2, confidence, class]
            ...
            if detection.any() and len(detection[0]) >= 6:
                '''try:
                    data = coordinate_format.format(center_x, center_y, z, 0, 0, 0)            # sock.sendall(data.encode())
                    sock.sendall(data.encode())
                except BrokenPipeError:
                    print("Connection to WebSocket server lost. Reconnecting...")
                    # Try to reconnect to the WebSocket server here
                    sock.connect((ur_robot_ip, ur_robot_port))
                    sock.sendall(data.encode())'''
                rob.movel((center_x,center_y,z,0,0,0), acc=acc, vel=vel)
                rob.movel((center_x,center_y,z+1,0,0,0), acc=acc, vel=vel) #down for pick, please setting
                rob.set_standard_digital_out(1, True)
                rob.movel((center_x,center_y,z,0,0,0), acc=acc, vel=vel)
